...
This article will guide you through the process of creating an architecture diagram for a 2-tier bookstore application using Python and the [`diagrams`] library.
...
We'll use the [`diagrams`] Python library, which allows for creating cloud system architecture diagrams using code. It supports various providers, including AWS, GCP, Azure, and more.
...
```python
    with Cluster("CI/CD Pipeline"):
        github = Python("GitHub Actions")
        pipeline = Codepipeline("CodePipeline")
        build = Codebuild("CodeBuild")
```

- **Cluster**: Groups components logically. Here, it groups the CI/CD pipeline components.
- **GitHub Actions**: Represented using a generic programming language icon (`Python`).
- **CodePipeline and CodeBuild**: Represent AWS services used for the CI/CD pipeline.
...
- **Load Balancer (ELB)**: Distributes traffic between frontend and backend services.
...
- **Connections**: Represented using `>>` operator, defining the flow and connections between components.
  - **client >> dns >> lb**: The user accesses the DNS service, which then directs traffic to the load balancer.
  - **github >> pipeline >> build**: Represents the CI/CD pipeline flow from GitHub Actions to AWS CodePipeline and then to CodeBuild.
  - **build >> lb**: The build process deploys updates to the load balancer.
  - **lb >> frontend** and **lb >> backend**: The load balancer directs traffic to both the frontend and backend services.
  - **backend >> db**: The backend service connects to the MongoDB database.
